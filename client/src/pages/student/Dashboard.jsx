...
const StudentDashboard = () => {
  ...
  try {
    ...
    // Update state and compute other metrics
  ...
  // ✅ Recommend Courses Locally (no backend dependency)
...
// ✅ Generate streak and actual daily learning hours
const generateConsecutiveStreak = (progressList) => {
  const today = new Date();
  const last7Days = [];
  const dailyMap = {};

  progressList.forEach((course) => {
    if (!course.lastUpdated) return;

    const key = new Date(course.lastUpdated).toISOString().split("T")[0];
    if (!dailyMap[key]) dailyMap[key] = 0;
    dailyMap[key] += course.minutesWatched;
  });
  ...
  const mostActive = progressData.sort(
        ...
        <div className="bg-white shadow-xl rounded-2xl p-6 sm:p-8 lg:p-10 border border-gray-100 space-y-10">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Welcome back, {userData?.name || "Student"} — here’s your learning summary.
            </p>
          </div>

          {/* Stats Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <InsightCard title="Total Courses" value={totalCourses} color="bg-blue-100" />
            <InsightCard title="Completed Courses" value={completedCourses} color="bg-green-100" />
            <InsightCard title="Certificates" value={totalCerts} color="bg-yellow-100" />
            <InsightCard title="Average Completion" value={`${avgCompletionRate}%`} color="bg-cyan-100" />
            <InsightCard title="Total Lectures" value={totalLectures} color="bg-purple-100" />
            <InsightCard title="Current Streak" value={`${streakCount} days`} color="bg-red-100" />
          </div>

          {/* Two Visualizations: Pie + Daily Streak */}
          ...
          {/* Recommended Courses */}
{(recommendations && recommendations.length > 0 ? recommendations.slice(0, 3) : allCourses.slice(0, 3)).length > 0 && (
  <div>
    <h2 className="text-xl font-semibold text-gray-800 mb-4">
      Recommended for You
    </h2>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {(recommendations.length > 0 ? recommendations.slice(0, 3) : allCourses.slice(0, 3)).map((course) => (
